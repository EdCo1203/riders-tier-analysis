...
def guardar_decision(rider_id, nombre, tier, score, semana, fallos_str, decision, mensaje, notas):
    ws = get_sheet()
    if ws is None:
    ...
    try:
        if not ws.get_all_values():
            ws.append_row(["Fecha Decisión", "Rider ID", "Nombre", "Tier", "Score",
                           "Semana", "Fallos", "Decisión", "Mensaje Enviado", "Notas"])
        ws.append_row([
            datetime.now().strftime("%Y-%m-%d %H:%M"),
            str(rider_id), nombre, tier, str(score),
            semana, fallos_str, decision, mensaje, notas
        ])
        return True
    except Exception as e:
...
<style>
    ...
    .rider-nombre { font-size: 1.05rem; font-weight: 700; color: #f5f5f5; margin-bottom: 0.2rem; }
    .rider-meta   { font-size: 0.78rem; color: #888; margin-bottom: 0.7rem; }
    .fallo-badge { display: inline-block; background: #2a1a1a; color: #f87171; border: 1px solid #ef444440; border-radius: 6px; padding: 2px 10px; font-size: 0.75rem; margin: 2px 3px 2px 0; }
    .day-row { background: #111; border: 1px solid #222; border-radius: 8px; padding: 0.6rem 1rem; margin: 0.3rem 0; font-size: 0.8rem; }
    .day-label { color: #888; font-weight: 600; margin-bottom: 0.3rem; }
    .metric-row { display: flex; gap: 0.6rem; flex-wrap: wrap; margin: 0.3rem 0; }
    .metric-item { background: #222; border-radius: 6px; padding: 3px 10px; font-size: 0.75rem; color: #bbb; }
    .metric-item.mal { color: #f87171; background: #2a1212; border: 1px solid #ef444430; }
    .metric-item.ok  { color: #34d399; }
    .msg-box { background: #111; border: 1px solid #2a2a2a; border-radius: 8px; padding: 0.9rem 1rem; font-size: 0.82rem; color: #ccc; white-space: pre-wrap; font-family: monospace; line-height: 1.6; }
    ...
    .decision-card { background: #1a1a1a; border: 1px solid #2a2a2a; border-radius: 12px; padding: 1.2rem 1.5rem; margin-bottom: 0.5rem; }
...
UMBRALES = {
    ...
    "Pedidos":      {"op": "<",  "val": None, "label": "Pedidos insuficientes", "desc": "Completó menos pedidos de los esperados"},
...
# STATS TOP
# ─────────────────────────────────────────
total_riders = len(riders_orden)
criticos     = sum(1 for r in riders_orden if df_filtered[df_filtered["rider_id"] == r]["n_fallos"].max() >= 3)
con_fallos   = sum(1 for r in riders_orden if df_filtered[df_filtered["rider_id"] == r]["n_fallos"].sum() > 0)
...
with c2: st.markdown(f'<div class="stat-box"><div class="stat-num rojo">{criticos}</div><div class="stat-label">Con día crítico (3+ fallos)</div></div>', unsafe_allow_html=True)
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "📋 Diagnóstico día a día", "💬 Mensajes", "✅ Decisiones", "📜 Histórico", "📊 Exportar"
])
...
with tab1:
    ...
    for rid in riders_orden:
        ...
        total_f  = df_rider["n_fallos"].sum()
        max_f    = df_rider["n_fallos"].max()
        if solo_fallos_tab1 and total_f == 0: continue
        if max_f < filtro_min: continue
        ...
        for _, dia in df_rider.iterrows():
            ...
            b = "".join([f'<span class="fallo-badge">{UMBRALES[f]["label"]}</span>' for f in fd]) if fd else '<span style="color:#34d399;font-size:0.73rem">✅ OK</span>'
        ...
        st.markdown(f'<div class="rider-card {clase}"><div class="rider-nombre">{nombre}</div><div class="rider-meta">ID: {rid} &nbsp;·&nbsp; {tier} &nbsp;·&nbsp; Score: {score} &nbsp;·&nbsp; {contrato}h &nbsp;·&nbsp; {vehiculo} &nbsp;·&nbsp; <b>{total_f} fallos en la semana</b></div>{dias_html}</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    for rid in riders_orden:
        ...
        if solo_fallos_tab2 and total_fallos == 0: continue
        ...
        for _, dia in df_rider.iterrows():
            for fallo in dia["fallos"]:
                if fallo not in fallos_semana:
                    fallos_semana[fallo] = dia["detalle_fallos"].get(fallo, {})
        if not fallos_semana: continue
...
with tab3:
    ...
    st.caption(f"Semana analizada: **{semana_str}**")

    canal_dec     = st.radio("Canal del mensaje", ["WhatsApp", "Email"], horizontal=True, key="canal_dec")
    canal_dec_key = "ws" if canal_dec == "WhatsApp" else "email"

    riders_con_fallos_list = [
        r for r in riders_orden
        if df_filtered[df_filtered["rider_id"] == r]["n_fallos"].sum() > 0
    ]

    if not riders_con_fallos_list:
        st.info("No hay riders con fallos en el período seleccionado.")
    else:
        for rid in riders_con_fallos_list:
            df_rider     = df_filtered[df_filtered["rider_id"] == rid].sort_values("day")
            total_fallos = int(df_rider["n_fallos"].sum())
            n_max        = int(df_rider["n_fallos"].max())
            nombre       = df_rider["Nombre"].iloc[0] if pd.notna(df_rider["Nombre"].iloc[0]) else f"Rider {rid}"
            tier         = df_rider["Tier"].iloc[0]   if "Tier"  in df_rider.columns else "—"
            score        = df_rider["Score"].iloc[0]  if "Score" in df_rider.columns else "—"
            ...
            fallos_str = " | ".join([UMBRALES[f]["label"] for f in fallos_semana.keys()])
            mensaje    = generar_mensaje_semanal(nombre, fallos_semana, canal_dec_key)
            icono      = "🔴" if n_max >= 3 else "🟡" if n_max == 2 else "🔵"
            borde      = "#ef4444" if n_max >= 3 else "#f59e0b" if n_max == 2 else "#3b82f6"

            st.markdown(f"""
            <div class="decision-card" style="border-left:4px solid {borde}">
                <div class="rider-nombre">{icono} {nombre}</div>
                <div class="rider-meta">ID: {rid} &nbsp;·&nbsp; {tier} &nbsp;·&nbsp; Score: {score} &nbsp;·&nbsp; {total_fallos} fallos esta semana</div>
                <div>{"".join([f'<span class="fallo-badge">{UMBRALES[f]["label"]}</span>' for f in fallos_semana.keys()])}</div>
            </div>
            """, unsafe_allow_html=True)

            st.markdown(f'<div class="msg-box">{mensaje}</div>', unsafe_allow_html=True)

            notas = st.text_input("Notas (opcional)", key=f"notas_{rid}", placeholder="Ej: ya habló con él, tiene problema de zona...")

            col_c, col_p = st.columns(2)
            with col_c:
                if st.button("✅ Contactar", key=f"contactar_{rid}", use_container_width=True):
                    ok = guardar_decision(rid, nombre, tier, score, semana_str, fallos_str, "Contactar", mensaje, notas)
                    if ok:
                        st.success(f"Guardado: {nombre} → Contactar")
            with col_p:
                if st.button("❌ Prescindir", key=f"prescindir_{rid}", use_container_width=True):
                    ok = guardar_decision(rid, nombre, tier, score, semana_str, fallos_str, "Prescindir", mensaje, notas)
                    if ok:
                        st.warning(f"Guardado: {nombre} → Prescindir")
...
with tab4:
    ...
    if df_hist.empty:
        st.info("Aún no hay decisiones guardadas en Google Sheets.")
    else:
        col_f1, col_f2 = st.columns(2)
        with col_f1:
            filtro_dec = st.multiselect("Filtrar por decisión", ["Contactar", "Prescindir"], default=["Contactar", "Prescindir"])
        with col_f2:
            if "Semana" in df_hist.columns:
                semanas = df_hist["Semana"].dropna().unique().tolist()
                filtro_sem = st.multiselect("Filtrar por semana", semanas, default=semanas)
            else:
                filtro_sem = []
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        cols_mostrar = [c for c in df_hist_f.columns if c != "Mensaje Enviado"]
        st.dataframe(df_hist_f[cols_mostrar], use_container_width=True, hide_index=True)

        st.download_button(
            label="⬇️ Descargar histórico Excel",
            data=to_excel(df_hist_f),
